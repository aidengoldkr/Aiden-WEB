...
type Platform = "soomgo" | "techBlog" | "linkedin" | "instagram" | "github" | "gmail";

const getPlatform = (label: string): Platform => {
  const normalized = label.toLowerCase();

  if (normalized.includes("soomgo")) return "soomgo";
  if (normalized.includes("tech blog")) return "techBlog";
...
function PlatformIcon({ platform }: { platform: Platform }) {
  ...
  if (platform === "instagram") {
    return (
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden="true">
        <path d="M7.75 2h8.5A5.75 5.75 0 0122 7.75v8.5A5.75 5.75 0 0116.25 22h-8.5A5.75 5.75 0 012 16.25v-8.5A5.75 5.75 0 017.75 2zm0 2A3.75 3.75 0 004 7.75v8.5A3.75 3.75 0 007.75 20h8.5A3.75 3.75 0 0020 16.25v-8.5A3.75 3.75 0 0016.25 4h-8.5z" fill="currentColor" />
        <path d="M12 7a5 5 0 110 10 5 5 0 010-10zm0 2a3 3 0 100 6 3 3 0 000-6zM17.25 6.5a1.25 1.25 0 110 2.5 1.25 1.25 0 010-2.5z" fill="currentColor" />
      </svg>
    );
  }
  ...
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <path d="M4 5h16a2 2 0 012 2v10a2 2 0 01-2 2H4a2 2 0 01-2-2V7a2 2 0 012-2zm0 3.2V17h16V8.2l-7.43 5.07a1 1 0 01-1.14 0L4 8.2zm1.15-1.2L12 11.67 18.85 7H5.15z" fill="currentColor" />
    </svg>
  );
}
  ...
  return (
    <section className={styles.elsewhere} id="elsewhere">
      <div className={styles.elsewhereContent}>
        <h3 className={styles.groupTitle} data-reveal="soft">CONTACT</h3>
          ...
          {socialData.map((item, index) => {
            ...
            return (
              <a
                key={index}
                href={item.url}
                className={`${styles.linkCard} ${styles[`${platform}Card`]}`}
                data-reveal="card"
